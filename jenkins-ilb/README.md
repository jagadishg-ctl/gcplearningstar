...
- **Private Key**: Terraform variable `ssl_private_key`
- **Certificate**: Terraform variable `ssl_certificate`
...
2. VPC `vpc-jenkins-private` must exist
3. Subnet `subnet-jenkins` must exist
4. SSL certificate values must be provided at runtime:
   - `ssl_private_key` (PEM private key)
   - `ssl_certificate` (PEM certificate chain)
...
2. Review the planned changes:
   ```bash
   terraform plan \
     -var="ssl_private_key=$(cat ../cert/jenkins.key)" \
     -var="ssl_certificate=$(cat ../cert/fullchain.pem)"
   ```
...
### GitHub Actions
Store certificate values in repository secrets and map them to Terraform variables:
- `JENKINS_SSL_PRIVATE_KEY` -> `TF_VAR_ssl_private_key`
- `JENKINS_SSL_CERTIFICATE` -> `TF_VAR_ssl_certificate`
...
- The load balancer terminates HTTPS and forwards HTTP traffic to Jenkins on port 80
